...
const srcPath = {
    css: 'src/sass/**/*.scss',
    img: 'src/images/**',// src imagesの中身全てを圧縮。images以下のフォルダ内のファイル圧縮はimg: 'src/images/**/*'n
    html: './**/*.html'
}
...
const cssdeclsort = require('css-declaration-sorter'); //cssをアルファベット順に並び替える
const gcmq = require('gulp-group-css-media-queries');//メディアクエリ　纏める
const browsers = [
    'last 2 versions',
    '> 5%',
    'ie = 11',
...
const cssSass = () => {
    return src(srcPath.css)
        ...
        .pipe(postcss([cssdeclsort({order: 'alphabetical'})])) // cssをアルファベット順に並べる smacss alphabetical が選べる
        .pipe(gcmq())
